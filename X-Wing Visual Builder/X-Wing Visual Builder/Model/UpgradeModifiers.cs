...
namespace X_Wing_Visual_Builder.Model
{
    static class UpgradeModifiers
    {
        ...
        {
            // A-Wing Test Pilot
            if (isRemovingUpgrades == false && pilot.upgrades.Contains(Upgrades.upgrades[11001]) && upgrade.upgradeType == UpgradeType.Elite && pilot.upgrades.Contains(upgrade)) { return true; }
            // TIE Shuttle
            else if (pilot.upgrades.Contains(Upgrades.upgrades[11031]) && upgrade.upgradeType == UpgradeType.Crew && upgrade.cost > 4) { return true; }
            // Royal Guard TIE
            else if (isRemovingUpgrades == false && pilot.upgrades.Contains(Upgrades.upgrades[11034]) && upgrade.upgradeType == UpgradeType.Modification && pilot.upgrades.Contains(upgrade)) { return true; }
            // Lightweight Frame
